...
// newStdLog returns logger that is compatible with the Logger interface
func newStdLog(cfg *Config) Logger {
	var flags int
...
func (l *stdLogger) setStdPrefix(level Level) {
	l.stdlog.SetPrefix("[" + config.SERVICENAME + ":" + level.String() + "] ")
}

func (l *stdLogger) setErrPrefix(level Level) {
